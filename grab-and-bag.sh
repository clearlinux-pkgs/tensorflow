...
pushd tensorflow-1.0.0-rc0
	cat ../powf.patch | patch -p1
	./configure < ../answers.txt
	bazel clean
	bazel build -c opt --copt=-g1 --copt=-Wno-sign-compare //tensorflow/tools/pip_package:build_pip_package
	...
	./configure < ../answers.txt
	bazel clean
	bazel build -c opt --copt=-mavx2 --copt=-O3 --copt=-march=haswell --copt=-mfma --copt=-g1 --copt=-Wno-sign-compare   //tensorflow/tools/pip_package:build_pip_package
	bazel-bin/tensorflow/tools/pip_package/build_pip_package $PWD/../tmp/
